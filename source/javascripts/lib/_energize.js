...
(function() {
    ...
    isThresholdReached = function(startXY, xy) {
        return (
            Math.abs(startXY[0] - xy[0]) > 5 || Math.abs(startXY[1] - xy[1]) > 5
        );
    };
    ...
    click = function(e) {
        ...
        let time = Date.now(),
            ...
            x = e.clientX,
            y = e.clientY;
        let xyDiff = [Math.abs(lastClick.x - x), Math.abs(lastClick.y - y)];
        let target = closest(e.target, "A") || e.target; // needed for standalone apps
        let nodeName = target.nodeName;
        let isLink = nodeName === "A";
        let standAlone =
